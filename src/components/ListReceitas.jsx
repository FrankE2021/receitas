...
const ListReceitas = () => {
  ...
  return (
    ...
    <div className={`bg-primary ${styles.paddingX} ${styles.flexCenter} flex-grow`}>
      <div className={`${styles.boxWidth}`}>
        <h1 className="border p-1 rounded-[10px] mb-5 flex justify-center items-center font-poppins text-[25px] font-extrabold text-gradient">RECEITAS:</h1>
        <ul>
          {receitas.map(receita => (
            <li key={receita.id}>
              <h2 className='flex justify-center text-gradient text-[25px]'>{receita.paciente.nome} {receita.paciente.idade}</h2>
            </li>
          ))}
      </ul>
